...
//*
$(function(){
	$('input:password[pwdpower]').each(function(index, element){
		var $this = $(this);
		$this.bind('input propertychange change keyup', function(){
			EvalPwd($this.val(), $this.attr('pwdpower'));
		});
	});
})
var level = "";
...
function EvalPwd(sP, domId)
{
    ...
    if(domId){
    	var $pwdpower = $('#' + domId);
    	$('.pweak,#pweak', $pwdpower).css('background', aPQ[0]);
    	$('.pmedium,#pmedium', $pwdpower).css('background', aPQ[1]);
    	$('.pstrong,#pstrong', $pwdpower).css('background', aPQ[2]);
    }else{
        $('#pweak').css('background', aPQ[0]);
    	$('#pmedium').css('background', aPQ[1]);
    	$('#pstrong').css('background', aPQ[2]);
    }
}
//*
...
{
return (IsLongEnough(ClientSideStrongPassword.arguments[0], "8") &&
